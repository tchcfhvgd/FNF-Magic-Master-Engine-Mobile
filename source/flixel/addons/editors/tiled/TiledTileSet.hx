...
class TiledTileSet
{
	...
	public function new(data:FlxTiledTileAsset, rootPath:String = "")
	{
		...
		{
			this.rootPath = Path.normalize(rootPath + source.att.source);

			var lroot:Array<String> = this.rootPath.split("/");
			var file:String = lroot.pop();

			this.rootPath = "";
			for (i in lroot){this.rootPath += '$i/';}

			if (Paths.exists('${this.rootPath}$file'))
			{
				source = new Access(Xml.parse('${this.rootPath}$file'.getText()));
				source = source.node.tileset;
			}
			else
			{
				throw 'Invalid TSX tileset path: ${this.rootPath}$file';
			}
		}
